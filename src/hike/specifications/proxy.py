...
class FieldPathBase(ABC):
    """Abstract base for field path objects that support specification building via comparison operators.

    Subclasses must implement :attr:`path`.  All comparison operators (``==``, ``!=``,
    ``<``, ``<=``, ``>``, ``>=``) and :meth:`matches` are provided here so that any
    concrete ``FieldPath`` gains a full specification-building DSL for free.
    """
    ...
    def _reject_proxy_operand(self, other: object) -> None:
        if isinstance(other, FieldPathBase):
            raise TypeError(
...
class FieldByName(FieldPathBase):
...
class TerminalFieldProxy(FieldPathBase):
