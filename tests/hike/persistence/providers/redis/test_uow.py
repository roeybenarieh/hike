...
@pytest.fixture
def repo(redis_client: Redis) -> RedisRepository[UUID, Boat]:
    return RedisRepository(redis_client, Boat, KEY_PREFIX)


@pytest.fixture
def uow(redis_client: Redis) -> UnitOfWork[Pipeline]:
    ctx = RedisDBContext(redis_client)
    return UnitOfWork(ctx)


@pytest.fixture
def journey_repo(redis_client: Redis) -> RedisRepository[UUID, Journey]:
...
def test_save_and_get_one(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_update(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_get_many_with_spec(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_count_with_spec(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    boat_a = Boat(name=Name("Alpha"), price=Price(10.0))
    boat_b = Boat(name=Name("Beta"), price=Price(50.0))
...
def test_upsert_creates_then_updates(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_delete(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_save_duplicate_raises(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_get_one_missing_raises(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_delete_missing_raises(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_rollback_on_exception(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_optimistic_lock_conflict(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
def test_journey_save_and_get_one_with_checkpoints(
    uow: UnitOfWork[Pipeline],
    journey_repo: RedisRepository[UUID, Journey],
) -> None:
...
@pytest.fixture
def fleet(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> list[Boat]:
    boats = [
...
def test_redis_ordering_price_asc(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
...
@pytest.mark.usefixtures("fleet")
def test_redis_ordering_price_desc(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
...
@pytest.mark.usefixtures("fleet")
def test_redis_offset_pagination(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
        page = repo.get_many(
            Boat.price >= 0.0,
...
@pytest.mark.usefixtures("fleet")
def test_redis_page_pagination(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
        page = repo.get_many(
            Boat.price >= 0.0,
            ordering=[asc(Boat.price)],
...
@pytest.mark.usefixtures("fleet")
def test_redis_cursor_pagination_traverses_all(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
...
@pytest.mark.usefixtures("fleet")
def test_redis_ordering_single_orderby_shorthand(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
        results = repo.get_many(Boat.price >= 0.0, ordering=asc(Boat.price))
...
@pytest.mark.usefixtures("fleet")
def test_redis_get_many_no_pagination_returns_list(uow: UnitOfWork[Pipeline], repo: RedisRepository[UUID, Boat]) -> None:
    with uow(repo):
